...
const isAstNode = (
  value: unknown
): value is { type: "expression" | "statement" } => {
  return (
    typeof value === "object" &&
    value !== null &&
    "type" in value &&
...
const assignSerialized = (
  ...
  if (Array.isArray(value)) {
    result[key] = value.map((item) =>
      isAstNode(item) ? serializeAstNode(item) : item
    );
    return;
  }
  if (isAstNode(value)) {
...
const serializeAstNode = (node: {
  ...
  if (
    typeof (node as { tokenLiteral?: () => unknown }).tokenLiteral ===
    "function"
  ) {
    ...
    case "PrefixExpression":
      ...
      if ("operator" in node) {
        assignSerialized(
          ...
          handled,
          "operator",
          (node as { operator: unknown }).operator
        );
      }
      break;
...
const serializeProgram = (program: {
  statements: Array<{ [key: string]: unknown }>;
}) => {
  return {
...
describe("Parser", () => {
  ...
  const ifElseSource =
    "if (count > 0) { total = total + count; } else { total = 0; }";
  const whileSource = "while (count < limit) { count = count + 1; }";
  const forSource = "for (let i: i32 = 0; i < 3; i = i + 1) { sum = sum + i; }";
  ...
  const expectVariableStatement = (stmt: unknown): LetStatement => {
    ...
    assert.ok(
      variableStmt.identifier,
      "missing identifier on variable statement"
    );
    assert.ok(
  ...
  test("parses standalone if statement", () => {
    ...
    assert.ok(
      ifStmt.thenBlock.statements.length > 0,
      "then block should contain statements"
    );
  ...
  test("parses standalone if/else statement", () => {
    ...
    assert.ok(
      ifStmt.elseBlock?.statements.length,
      "else block should contain statements"
    );
  });
  ...
  test("parses standalone while loop", () => {
    ...
    assert.ok(
      whileStmt.loopBody instanceof BlockStatement,
      "missing loop body block"
    );
    assert.ok(
  ...
  test("parses standalone for loop", () => {
    ...
    assert.ok(
      forStmt.initBlock instanceof LetStatement,
    ...
    assert.ok(
      forStmt.loopBody instanceof BlockStatement,
      "missing loop body block"
    );
    assert.ok(
  ...
  describe("error recovery", () => {
    test("reports missing delimiter in let statement", () => {
      ...
      assert.ok(
        errors.some(
          (msg) => /i32/i.test(msg) || /type/.test(msg.toLowerCase())
        ),
    ...
    test("collects multiple errors for missing semicolons", () => {
      ...
      const semicolonMentions = errors.filter(
        (msg) => msg.toLowerCase().includes("semicolon") || msg.includes(";")
      );
      assert.ok(
