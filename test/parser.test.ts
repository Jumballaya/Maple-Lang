...
describe("Emission: Struct Let", () => {
  test("struct let with mixed i32 and f32 members emits both", () => {
    ...
    `);
    assert(wat.includes("p_x"), "WAT should contain the integer member p_x");
    assert(wat.includes("p_y"), "WAT should contain the float member p_y");
  });
  ...
  test("struct let with only f32 members emits all members", () => {
    ...
    `);
    assert(wat.includes("v_x"), "WAT should contain v_x");
    assert(wat.includes("v_y"), "WAT should contain v_y");
  });
});
...
describe("Emission: Member Access", () => {
  test("member access with identifier parent compiles", () => {
    ...
    `);
    assert(wat.includes("(func"), "WAT should contain a function");
  });
...
describe("Emission: Function Params", () => {
  test("two-param function emits exactly two param declarations", () => {
    const { wat } = compile(`
      export fn add(a: i32, b: i32): i32 {
        return a + b;
      }
    `);
    const paramMatches = wat.match(/\(param \$/g);
    assert(paramMatches !== null, "WAT should contain param declarations");
    assert.equal(
      paramMatches.length,
      2,
      `Expected 2 params, got ${paramMatches.length}`
    );
  });

  test("four-param function emits exactly four param declarations", () => {
    const { wat } = compile(`
    ...
    assert.equal(
      paramMatches.length,
      4,
      `Expected 4 params, got ${paramMatches.length}`
    );
  });
});
...
describe("Module Exports", () => {
  test("exported struct appears in exports", () => {
    const p = new Parser("export struct Visible { x: i32 }");
    const ast = p.parse("test");
    assert(p.errors.length === 0);
    const meta = extractModuleMeta(ast);
    assert(
      meta.exports["Visible"] !== undefined,
      "Visible should be in exports"
    );
    assert.equal(meta.exports["Visible"].kind, "struct");
  });

  test("non-exported struct does not appear in exports", () => {
    const p = new Parser("struct Hidden { x: i32 }");
    const ast = p.parse("test");
    assert(p.errors.length === 0);
    const meta = extractModuleMeta(ast);
    assert(
      meta.exports["Hidden"] === undefined,
      "Hidden should NOT be in exports"
    );
  });

  test("mixed exported and non-exported structs", () => {
    const p = new Parser(`
      export struct Pub { x: i32 }
      struct Priv { y: f32 }
    `);
    const ast = p.parse("test");
    assert(p.errors.length === 0);
    const meta = extractModuleMeta(ast);
    assert(meta.exports["Pub"] !== undefined, "Pub should be in exports");
    assert(
      meta.exports["Priv"] === undefined,
      "Priv should NOT be in exports"
    );
    assert(meta.structs["Pub"] !== undefined, "Pub should still be in structs");
    assert(
      meta.structs["Priv"] !== undefined,
      "Priv should still be in structs"
    );
  });
});

describe("Compiler", () => {
  test("accepts output path parameter", () => {
    assert.equal(typeof compiler, "function");
    assert(
      compiler.length >= 3,
      "compiler should accept at least 3 required params"
    );
  });

  test("reports file errors for missing input", async () => {
    await assert.rejects(
      compiler("/nonexistent/path.maple", "path", "/nonexistent", "out.wasm"),
      (err: Error) => {
        assert(
          err.message.includes("ENOENT"),
          "Should fail on file I/O"
        );
        return true;
      }
    );
